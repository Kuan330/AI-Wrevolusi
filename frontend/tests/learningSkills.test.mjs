...
const source=readFileSync(new URL('../src/pages/Skills/learningSkills.ts',import.meta.url),'utf8').replace('import { accountStorage } from "../../services/accountStorage.ts";', 'const accountStorage = {getItem: (key) => key === "aiwrevolusi.confirmedAnalysis" ? "current-work" : globalThis.skillData ?? null, setItem: (_key, value) => {globalThis.skillData = value}};');
...
test('explicit WEF additions remain alongside current work',()=>{const extra=m.growingSkills[0];assert.deepEqual(m.reconcileLearningSkills([extra],[]),[extra])});
