...
export type PlanState = {
  version: 1;
  revision: number;
  /** Snapshot of the occupation/task context used when this plan was made. */
  context?: string;
  events: PlanEvent[];
};
...
export const timeString = (minutes: number) =>
  `${String(Math.floor(minutes / 60)).padStart(2, "0")}:${String(minutes % 60).padStart(2, "0")}`;
export const duration = (event: PlanEvent) =>
  mins(event.end) - mins(event.start);
export const isMine = (event: PlanEvent) =>
  event.assistance?.status !== "accepted";
export const overlaps = (a: PlanEvent, b: PlanEvent) =>
  a.id !== b.id &&
  a.date === b.date &&
  mins(a.start) < mins(b.end) &&
  mins(b.start) < mins(a.end) &&
  isMine(a) &&
  isMine(b);
export function conflicts(events: PlanEvent[]) {
...
export function validateEvent(e: PlanEvent) {
  ...
  if (
    ...
    Number.isNaN(new Date(`${e.date}T12:00:00`).valueOf()) ||
    dateKey(new Date(`${e.date}T12:00:00`)) !== e.date
  )
...
export function suggestions(event: PlanEvent, events: PlanEvent[]) {
  ...
  for (let day = 0; day < 7 && result.length < 3; day++)
    for (
      let start = 8 * 60;
      start + duration(event) <= 21 * 60 && result.length < 3;
      start += 30
    ) {
      const next = {
        ...event,
        date: addDays(event.date, day),
        start: timeString(start),
        end: timeString(start + duration(event)),
        assistance: undefined,
      };
      if (new Date(`${next.date}T${next.start}`) <= new Date()) continue;
      if (next.date === event.date && next.start === event.start) continue;
...
export function demoPlan(): PlanState {
  ...
  return {
    ...
    events: [
      ...[0, 1, 2, 3, 4].map((d) =>
        make(`work-${d}`, "Work", "work", d, "09:00", "17:00"),
      ),
      {
        ...make("learn-1", "Check AI answers", "learning", 1, "18:30", "19:00"),
        resourceId: "demo-fact-check",
      },
      {
        ...make(
          "learn-2",
          "Practise a useful work prompt",
          "learning",
          3,
          "18:30",
          "19:00",
