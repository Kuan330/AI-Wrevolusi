...
import { loadSavedPossibilities, possibilitiesProfilePath, toPossibilitiesData, type PossibilitiesData } from "./possibilitiesModel";
...
export default function Possibilities() {
    ...
    void loadSavedPossibilities({
      ...
      onSuccess: r => {
        ...
        setSelectedCode(current => {
          ...
          return r.directions.some(direction => direction.occupation_code === candidate) ? candidate : null;
        });
        setShortlist(r.shortlisted_skill_ids.filter(id => r.skills.some(skill => skill.skill_id === id)).slice(0, 60));
      },
      onError: error => {
        setError(error instanceof Error ? error.message : "Could not save your work profile or load career exploration. Please try again.");
        setLoading(false);
      },
    });
  ...
  if (error || !data) return <div className="px-page"><p role="alert">{error || "Possibilities are unavailable."}</p><Link to={profilePath}>Review your work profile</Link></div>;
  if (data.status === "needs_profile") return <div className="px-page"><PageHeader title="Possibilities" description="Explore directions based on your work profile." /><section className="px-no-direction"><h2>Complete your Work Profile</h2><p>Confirm your current role and tasks first so we can show relevant directions.</p><Link className="px-primary" to={profilePath}>Go to Work Profile <ArrowRight size={15} /></Link></section></div>;
