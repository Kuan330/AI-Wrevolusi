...
const TaskListCard = (props: TaskListCardProps) => {
  ...
  const taskExposureAssessmentByTaskId = new Map(
    taskExposureAssessments.map((assessment) => [
      assessment.task_id,
      assessment,
    ]),
  );
  ...
  useEffect(() => {
    ...
    tasks.length,
    taskExposureAssessments.length,
    scoreRange,
  ...
  return (
    <AnalysisCard
      ...
      headerContent={<ScoreRangeSlider {...scoreRangeSliderProps1} />}
      titleTone={titleTone}
