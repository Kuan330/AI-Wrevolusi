...
export function TimePicker({ value = '', onChange, label = 'Choose time', compact = false }: { value?: string; onChange: (value: string) => void; label?: string; compact?: boolean }) {
  const [open, setOpen] = useState(false);
  const hour = value.split(':')[0] || '18';
  const minute = value.split(':')[1] || '00';
  return <><button type="button" aria-label={label} aria-haspopup="dialog" onClick={() => setOpen(true)} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', width:'100%', padding:compact ? '8px 10px' : '12px 16px', border:'1px solid #cedee9', borderRadius:14, background:'#fff', color:'#4f91ba', fontSize:compact ? 13 : 16, whiteSpace:'nowrap', gap:6, height:compact ? 36 : undefined }}><span>{value || (compact ? '--:--' : 'Select time')}</span><Clock size={compact ? 14 : 18} style={{flexShrink:0}}/></button>
